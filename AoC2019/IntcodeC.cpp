...
class IntcodeC {
	...
	static long long resolveParam(map<long long, long long> &input, vector<int> &command, long long param, unsigned digit_idx) {
	...
	static bool handleOpcode(IntcodeProgram &prog, istream &s_in, ostream &s_out, bool haltOnOutput=false) {
		...
		switch (opcode) {
		case 1: // add
			input[resolveParam(input, command, idx, 3)] = getValue(input, resolveParam(input, command, idx, 1)) + getValue(input, resolveParam(input, command, idx, 2));
			idx = idx + 4;
			return false;
		case 2: // mul
			input[resolveParam(input, command, idx, 3)] = getValue(input, resolveParam(input, command, idx, 1)) * getValue(input, resolveParam(input, command, idx, 2));
		...
		case 4: // outputs
			if (&s_out == &cout)
				s_out << "Outputs: ";
			s_out << getValue(input, resolveParam(input, command, idx, 1)) << endl;
			idx = idx + 2;
			return haltOnOutput;
		case 5: // jump-if-true
			idx = getValue(input, resolveParam(input, command, idx, 1)) != 0 ? getValue(input, resolveParam(input, command, idx, 2)) : idx + 3;
			return false;
		case 6: // jump-if-false
			idx = getValue(input, resolveParam(input, command, idx, 1)) == 0 ? getValue(input, resolveParam(input, command, idx, 2)) : idx + 3;
			return false;
		case 7: // less than
			input[resolveParam(input, command, idx, 3)] = getValue(input, resolveParam(input, command, idx, 1)) < getValue(input,resolveParam(input, command, idx, 2)) ? 1 : 0;
			idx = idx + 4;
			return false;
		case 8: // equals
			input[resolveParam(input, command, idx, 3)] = getValue(input,resolveParam(input, command, idx, 1)) == getValue(input,resolveParam(input, command, idx, 2)) ? 1 : 0;
